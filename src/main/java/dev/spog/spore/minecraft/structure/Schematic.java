...
public class Schematic {
    ...
    public static final String FILE_EXTENSION = ".sporeschem";
    ...
    public Schematic(Iterable<Block> input,
                     int width,
                     int height,
                     int length) {
        ...
        try (Reader reader =
            ...
            if (version > FORMAT_VERSION) {
                throw new IllegalStateException(
                        "Unsupported future schematic version: "
                                + version
                );
            }
            ...
            s.blocks = decodeBlocks(
                    root.get("blocks").getAsString()
            );

            JsonArray paletteArray =
                    root.getAsJsonArray("palette");
    ...
    private static JsonObject migrate(JsonObject root,
                                      int oldVersion) {
    ...
    private static String encodeBlocks(int[] blocks) {
        ...
        ByteBuffer buf =
                ByteBuffer.allocate(blocks.length * 4);
    ...
    public boolean hasBlock(int worldX,
                            int worldY,
                            int worldZ,
                            int originX,
                            int originY,
